...
DCONF_PROFILE
    ...
    # Deploy release config — tells smplos-os-update where to fetch OS updates
    # Set SMPLOS_GITHUB_REPO to owner/repo before publishing the repository.
    mkdir -p "$airootfs/etc"
    cat > "$airootfs/etc/smplos-release.conf" << 'RELCONF'
# smplOS Release Configuration
# Configures the git-based update system (smplos-os-update).
# The repo is cloned to ~/.local/share/smplos/repo/ and updated via git pull.
SMPLOS_GITHUB_REPO="smpl-os/smplos"
RELCONF
    log_info "Release config deployed (/etc/smplos-release.conf)"
...
setup_boot() {
    ...
    # NOTE: Do NOT generate a custom GRUB font for the live ISO.
    # mkarchiso's grub-mkstandalone embeds its own unicode.pf2 and
    # overriding it can cause "syntax error" on some firmware.
    # The installed system gets a HiDPI font via install.sh instead.
...
###############################################################################
# Build Update Tarball (published alongside the ISO to GitHub Releases)
#
# Produces release/smplos-update-X.Y.Z.tar.gz — the artifact that
# smplos-os-update downloads to update an already-installed system.
# The tarball contains:
#   bin/      → /usr/local/bin/   (always updated, users never edit scripts)
#   config/   → ~/.config/        (smart: only if unmodified by user)
#   themes/   → ~/.local/share/smplos/themes/  (always updated)
#   manifest.txt  policy + path for every file
#   VERSION   version string
###############################################################################

# NOTE: build_update_tarball() was removed — smplOS now uses git-based updates.
# Users receive script/config updates via `smplos-os-update` which does `git pull`
# on ~/.local/share/smplos/repo/ and syncs files from there.

build_update_tarball() {
    # No-op: smplOS now uses git-based updates instead of tarball delivery.
    # Kept as a stub in case external scripts call it.
    log_info "Skipping update tarball — smplOS now uses git-based updates"
}
